...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        elif config.ssm_type == "S4D-Complex" or config.ssm_type == "S4D-Real":
            self.ssm_kernel = FFTConv(d_model=config.d_inner,
                                      d_state=config.d_state,
                                      activation='id',
                                      transposed=False,
                                      mode='s4d',
                                      is_real=False if config.ssm_type == "S4D-Complex" else True,
                                      shared=config.channel_sharing,
                                      init="diag-lin" if config.ssm_type == "S4D-Complex" else "legs",
